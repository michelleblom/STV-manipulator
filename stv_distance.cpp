	...
	const Config&, config, bool, isleaf, ostream&,log)
{
	...
	if(hasIncumbent()){
		...
		if(inc_obj - dual_obj <= 3){
			timer.aborted = true;
			abort();
		}

		if(inc_obj < timer.bestobj-3){
			timer.bestobj = inc_obj;
			GetTime(&timer.besttime);
		}
		else if(dual_obj > timer.best_dual + 3){
...
void Preliminaries(const Ballots &ballots, 
	...
	bool compbounds, const set<int> &elected_o, std::ostream &log){
...
void CreateEquivalenceClasses_RL(const Ballots &ballots, 
	const Candidates &cand, const Config &config, Node &node,
	const set<int> &elected_o, std::ostream &log, const Ints2d &rorder_c,
	const Ints &rorder_a){
	try{
			...
			for(int j = 0; j < ncand; ++j){
				...
				if(find(rorder_c[j].begin(), rorder_c[j].end(), i) !=
                    ...
                    if(config.addonly || config.deleteonly){
                        node.cand_equota[i] = min(lelim+1, last_q);
                    }
					break;
				}
			}
		...
		if(config.deleteonly){
            Ints c2pos(cand.size(), config.ncandidates);
            Ints2d nb_ids;
            for(int i = 0; i < cand.size(); ++i){
                c2pos[i] = node.cand_proc[i];
                //cout << "cand " << i << " proc " << c2pos[i] << endl;
                nb_ids.push_back(Ints());
            }
			...
			for(int i = 0; i < ballots.size(); ++i){
                ...
                // Reduce to EQ.
                Ints eqc;
                ReduceToEQ(config, blt.prefs, c2pos, eqc);
                /*Ints temp;
                ReduceToEQ(config, blt.prefs, c2pos, temp);
                for(int j = 0; j < eqc.size(); ++j){
                    cout << eqc[j] << ",";
                }
                cout << " --> "; 
                ...
                // check if ballot is already present
                Ints &cbs = nb_ids[eqc[0]];
   			    bool found_ballot = false;
	    		int bid = -1;
		    	for(int j = 0; j < cbs.size(); ++j){
			    	const Ballot &bf = node.rev_ballots[cbs[j]];
				    if(bf.prefs == eqc){
					    bid = bf.tag;
    					found_ballot = true;
	    				break;
	    		...
	    		if(found_ballot){
		    		Ballot &fb = node.rev_ballots[bid];
			    	fb.votes += blt.votes;
	    		}
		    	else{
                    Ballot nb(blt);
                    nb.prefs = eqc;
                    nb.tag = node.rev_ballots.size();

                    node.rev_ballots.push_back(nb);
                    cbs.push_back(nb.tag);
				
                    int first = eqc[0];
				    for(int l = 0; l < ncand; ++l){
					    node.poss_tally[first][l].insert(nb.tag);
			...
			for(int i = 0; i < node.remcand.size(); ++i){
                ...
                //node.newid2bids.push_back(Ints());
			}
			...
			for(int b = 0; b < ballots.size(); ++b){
                ...
                //node.newid2bids[rid].push_back(b);
			}
		}
...
double distance(const Ballots &ballots, const Candidates &cand, 
	...
	double tleft, bool lprelax, const set<int> &elected_o, ostream &log,
	const Ints &origc, const Ints &origa, double &def_ub, Result &ret)
{
	...
	try{
		...
		int rlast_q = rorder_c.size();
		int rlast_e = rorder_c.size(); 
        ...
        if(config.original_votes != -1){
            quota_lb = ((config.original_votes)/(config.original_seats+1));
            quota_ub = 1+((config.original_votes)/(config.original_seats+1));
        }

        //log << "Obj lb " << lb << " ub " << ub << endl;

        if(config.quota_override == -1){
		    if(config.deleteonly){
			    quota_lb=((config.original_votes-ub)/(config.original_seats+1));
		    }

		    if(config.addonly){
			    quota_ub=1+((config.original_votes+ub)/(config.original_seats+1));
		    }
            log << "Quota lb " << quota_lb << " quota ub " << quota_ub << endl;
        }
		...
		for(int i = 0; i < sigs; ++i){
			...
			ys[i] = IloNumVar(env, 0, ns+ub, ILOFLOAT, varname);
		...
		if(config.quota_override == -1){
            if(config.deleteonly){
			    cmodel.add(vquota >= (config.original_votes-obj)/
                    (config.original_seats+1));
			    cmodel.add(vquota <= 1+((config.original_votes-obj)/
                    (config.original_seats+1)));
            }
            else if(config.addonly){
			    cmodel.add(vquota >= ((config.original_votes+obj)/
		...
		for(int i = 0; i < config.ncandidates; ++i){
			...
			for(int j = 0; j < ncandr; ++j){
                //log << ncand << " " << config.ncandidates << " " << rlast_q << 
                //    " " << rlast_e << " " << j << endl;
				...
				double MV = config.totalvotes;
                if(config.deleteonly || config.addonly){
                    MV = config.original_votes;
                }
				...
				if(!ca){
					...
					if(config.deleteonly || config.addonly){
                        ...
                        //log << "6 v[" << i << "][" << j << "] >= vquota" << endl;
					}
					else{
						v_ij[i][j] = IloNumVar(env, config.quota, MV,
                            ILOFLOAT, varname);
					}
				}
				else{
				...
				if(s_setvalue[i][j] == 0){
					if(config.deleteonly || config.addonly){
                        //log << "7 v[" << i << "][" << j << "] < vquota" << endl;
						cmodel.add(v_ij[i][j] <= vquota);
					}
					else{
						cmodel.add(v_ij[i][j] <= min(MV,config.quota));
					}
				}
				else if(s_setvalue[i][j] == 1){
					if(config.deleteonly || config.addonly){
						cmodel.add(v_ij[i][j] >= vquota);
                        //log << "8 v[" << i << "][" << j << "] >= vquota" << endl;
					}
					else{
					...
					if(config.deleteonly || config.addonly){
						...
						cmodel.add(v_ij[i][j] <= znsij - 0.0001*nots_ij[i][j]
							+ s_ij[i][j] * MV);
						/*// Creating and adding constraint: v[i][j] >= s[i][j] Q
		...
		for(int i = 0; i < config.ncandidates; ++i){
			...
			for(int j = 0; j < ncandr; ++j){
				...
				for(SInts::const_iterator kt = ds.begin(); kt != ds.end();++kt){
					...
					double M = config.totalvotes;
                    if(config.deleteonly || config.addonly){
                        M = config.original_votes;
                    }
		...
		for(int j = 0; j < ncandr; ++j){
			...
			if(rorder_a[j]){
				...
				//      v[cand] - Q
				if(j != ncandr - 1){
					int MTV = config.totalvotes;
                    if(config.deleteonly || config.addonly){
                        MTV = config.original_votes;
                    }
					...
					if(config.deleteonly || config.addonly){
						...
						IloNumVar z1(env,0,config.original_votes,ILOFLOAT,varname);

						cmodel.add(z1 <= config.original_votes * tvgs);
						cmodel.add(z1 <= v_ij[c][j]);
						cmodel.add(z1 >= v_ij[c][j]-(1-tvgs)*config.original_votes);
						cmodel.add(z1 >= 0);
						...
						IloNumVar z2(env,0,config.original_votes, ILOFLOAT,varname);
						cmodel.add(z2 <= config.original_votes * tvgs);
						cmodel.add(z2 <= tw_k[j]);
						cmodel.add(z2 >= tw_k[j]-(1-tvgs) * config.original_votes);
        ...
        //cmodel.add(v_ij[6][0] <= 70670);
        //cmodel.add(v_ij[5][0] >= 5209);
		TimerRecord timer;
		timer.aborted = false;
		GetTime(&timer.start);
		GetTime(&timer.besttime);
	...
	//	}
		...
		if(cplex.getCplexStatus() == IloCplex::Infeasible){
            ...
            //double vq = cplex.getValue(vquota);

            //log << "Quota computed " << vq << endl;
            //log << "Total added " << cplex.getObjValue() << endl;
            //log << "Total votes " << cplex.getValue(totvotes) << endl;
            /*for(int i = 0; i < sigs; ++i){
               ...
               // log << "v[" << i << "] = " << cplex.getValue(ys[i]) << endl;
				if(p > 0){
					log << "(";
		...
		if(result && node.remcand.size() == 0){
			if(cplex.getBestObjValue() >= 0 &&
				cplex.getBestObjValue() <= ub){
                def_ub = ceil(max(lb, cplex.getBestObjValue()));
            }

            Manipulation mp;
            mp.order_c = node.order_c;
            mp.order_a = node.order_a;

            //Doubles r_votecounts(ballots.size(), 0);
            //for(int i = 0; i < ballots.size(); ++i){
            //   r_votecounts[i] = ballots[i].votes;
            //}

			for(int i = 0; i < sigs; ++i){
				const Ballot &bl = node.rev_ballots[i];
				int p = (!config.deleteonly) ? ceil(cplex.getValue(ps[i])) : 0;
				int m = (!config.addonly) ? ceil(cplex.getValue(ms[i])) : 0;
				...
				if(p > 0){
                    ...
                    mp.toadd.push_back(make_tuple(bl.prefs, p, -1, -1));
				}
				else if(m > 0){
					log << "(";
					for(int j = 0; j < bl.prefs.size(); ++j){
					...
					mp.todel.push_back(make_tuple(bl.prefs, m, -1, -1));
				}
			}


            ret.mps.push_back(mp);
		}
